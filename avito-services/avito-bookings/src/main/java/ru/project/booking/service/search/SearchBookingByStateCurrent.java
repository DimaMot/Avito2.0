...
public class SearchBookingByStateCurrent implements SearchBookingsByState {
    ...
    public List<Booking> getBookingsByState(long userId, BookingUserRole role, OffsetDateTime now) {
        return switch (role) {
            case OWNER -> {
                ...
                yield bookingRepository.findByOwnerIdAndStartBeforeAndEndAfterOrderByStartDesc(itemsIds, now, now);
            }
            case BOOKER -> bookingRepository.findByBookerIdAndStartBeforeAndEndAfterOrderByStartDesc(userId, now, now);
        };
    }
}
