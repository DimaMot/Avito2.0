...
@Component
public class SearchBookingByStatePast implements SearchBookingsByState {
    ...
    public List<Booking> getBookingsByState(long userId, BookingUserRole role, OffsetDateTime now) {
        return switch (role) {
            case OWNER -> {
                ...
                yield bookingRepository.findByOwnerIdAndEndBeforeOrderByStartDesc(itemsIds, now);
            }
            case BOOKER -> bookingRepository.findByBookerIdAndEndBeforeOrderByStartDesc(userId, now);
        };
    }
}
